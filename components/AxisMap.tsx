      ...
      <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-gray-200 p-3 w-52">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-1">
             {/* COPY (Active if a keyframe is selected) */}
             <button 
                onClick={copyKeyframeState}
                disabled={!selectedKf}
                className={`p-1 rounded transition-colors ${!selectedKf ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-blue-500 hover:bg-blue-50'}`}
                title={selectedKf ? "Copy State" : "Select a state to copy"}
             >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
             </button>

             {/* PASTE (Active if clipboard has data) */}
             <button 
                ...
                title={clipboard ? "Paste State" : "Clipboard empty"}
            >
