...
const Icons = {
  ...
  Shape: () => <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="3" rx="4"/></svg>,
...
export const Toolbar: React.FC = () => {
  const { 
    ui, setTool, undo, redo, history, toggleSettings, setMode, 
    setBrushColor, setFillColor, setBrushSize, setStrokeResolution, 
    closeAllPanels, toggleTransformMode, toggleSymmetry,
    toggleTimelinePanel, toggleInteractionsPanel, toggleExpertMode,
    toggleGallery, toggleShareModal
  } = useStore();
    ...
    <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50">
      <div className="bg-white/90 backdrop-blur-md border border-gray-200/50 shadow-xl rounded-full px-2 py-2 flex items-center gap-2 h-16 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`flex items-center gap-1 transition-opacity ${isPlayMode ? 'opacity-30 pointer-events-none grayscale' : 'opacity-100'}`}>
          ...
          <ToolButton tool="shape" icon={Icons.Shape} label="Shape / Rectangle" />
        ...
        {/* Expert Mode Controls (Timeline & Interactions) */}
          ...
          <>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                ...
                <span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
                  {ui.isTimelineOpen ? 'Masquer Timeline' : 'Afficher Timeline'}
                </span>
              </button>

              <button
                onClick={(e) => { e.stopPropagation(); toggleInteractionsPanel(); }}
                className={`p-2.5 rounded-full transition-all duration-200 flex items-center justify-center group relative ${
                  ui.isInteractionsOpen 
                    ? 'bg-purple-600 text-white shadow-md' 
                    : 'text-gray-600 hover:bg-gray-100'
                }`}
                title="State Machine & Interactions"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                <span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none">
                  {ui.isInteractionsOpen ? 'Masquer Interactions' : 'Interactions'}
                </span>
              </button>
            </div>
          </>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); toggleShareModal(); }}
            className={`p-2.5 rounded-full transition-all duration-200 flex items-center justify-center group relative ${
              ui.isShareOpen 
                ? 'bg-blue-600 text-white shadow-md' 
                : 'text-gray-600 hover:bg-gray-100'
            }`}
            title="Partager mon animation"
          >
